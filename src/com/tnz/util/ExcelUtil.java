...
public class ExcelUtil {
	 ...
	 * Excel文件路径
	 */
	private String excelPath = "default.xlsx";
	...
	/**
	 * 自动根据文件扩展名，调用对应的读取方法 不指定路径,则读取默认xls文件
	 * 
	 * @return
	 * @throws IOException
	 */
	 ...
	 * @throws IOException
	 */
	private void writeExcel_xls(List<Row> rowList, String dist_xlsPath)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	private void writeExcel_xls(List<Row> rowList, String src_xlsPath,
		...
		try {
			...
			// 将rowlist的内容写到Excel中
			writeExcel_wb(wb, rowList, dist_xlsPath);
	 ...
	 * @throws IOException
	 */
	private void writeExcel_xlsx(List<Row> rowList, String dist_xlsPath)
			throws IOException {
		writeExcel_xls(rowList, excelPath, dist_xlsPath);
	}
	 ...
	 * @throws IOException
	 */
	private void writeExcel_xlsx(List<Row> rowList, String src_xlsPath,
			String dist_xlsPath) throws IOException {
		// 判断文件路径是否为空
		if (dist_xlsPath == null || dist_xlsPath.equals("")) {
			out("文件路径不能为空");
	 ...
	 * @throws IOException
	 */
	private List<Row> readExcel_xlsx(String xlsPath) throws IOException {
		// 判断文件是否存在
		File file = new File(xlsPath);
		if (!file.exists()) {
	 ...
	 * @throws IOException
	 */
	private List<Row> readExcel_xls(String xlsPath) throws IOException {
		// 判断文件是否存在
		File file = new File(xlsPath);
		if (!file.exists()) {
			throw new IOException("文件名为" + file.getName() + "Excel文件不存在！");
	 ...
	 * @return
	 */
	private List<Row> readExcel_wb(Workbook wb) {
		List<Row> rowList = new ArrayList<Row>();
	 ...
	 * @param xlsPath
	 */
	public void writeExcel_wb(Workbook wb, List<Row> rowList, String xlsPath) {
		if (wb == null) {
			out("操作文档不能为空！");
			return;
		}
